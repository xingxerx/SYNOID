...
use crate::agent::core_systems::brain::{Brain, Intent};
use crate::agent::ai_systems::gpt_oss_bridge::SynoidAgent;
use crate::agent::ai_systems::multi_agent::DirectorAgent;
...
pub struct SuperEngine {
    ...
    api_url: String,
    pub animator: Option<std::sync::Arc<crate::agent::video_processing::animator::Animator>>,
}

impl SuperEngine {
    /// Initialize the Super Engine with all sub-systems
    pub fn new(api_url: &str, animator: Option<std::sync::Arc<crate::agent::animator::Animator>>) -> Result<Self, Box<dyn std::error::Error>> {
        info!("[SUPER_ENGINE] Initializing Synoid Unified Systems...");

        // Brain utilizes LLM
        let brain = Brain::new(api_url, "llama3:latest", animator.clone());
