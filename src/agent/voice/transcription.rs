...
pub trait TranscriptAnalysis {
    fn find_silence_near(&self, timestamp: f64) -> f32;
}

impl TranscriptAnalysis for Vec<TranscriptSegment> {
    fn find_silence_near(&self, timestamp: f64) -> f32 {
        // Find segment ending before timestamp
        let prev = self.iter().filter(|s| s.end <= timestamp).max_by(|a, b| {
            a.end
                .partial_cmp(&b.end)
                .unwrap_or(std::cmp::Ordering::Equal)
        });

        // Find segment starting after timestamp
        let next = self.iter().filter(|s| s.start >= timestamp).min_by(|a, b| {
            a.start
                .partial_cmp(&b.start)
                .unwrap_or(std::cmp::Ordering::Equal)
        });
        ...
        match (prev, next) {
            (Some(p), Some(n)) => (n.start - p.end).max(0.0) as f32,
...
/// Sovereign Ear (Alias for TranscriptionEngine)
pub type SovereignEar = TranscriptionEngine;

impl TranscriptionEngine {
    /// Initialize the Sovereign Ear with a specific model path or default
    pub fn new(model_path_override: Option<&str>) -> Result<Self> {
        // Locate or download the model
        let model_path = if let Some(p) = model_path_override {
            PathBuf::from(p)
        } else {
            Self::ensure_model("base.en")?
        };

        if !model_path.exists() && model_path_override.is_some() {
            // If override provided but missing, try to ensure default
            warn!(
                "Override model not found: {:?}. Falling back to base.en",
                model_path
            );
