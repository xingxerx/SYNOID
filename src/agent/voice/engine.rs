...
impl VoiceEngine {
    pub fn new() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        Ok(Self { device, model_dir, profiles_dir })
    }
    ...
    fn extract_voice_features(&self, audio_path: &Path) -> Result<Vec<f32>, Box<dyn std::error::Error>> {
        // Read WAV file
        let mut reader = hound::WavReader::open(audio_path)?;
        let spec = reader.spec();
        
        info!("[VOICE] Audio: {} Hz, {} channels", spec.sample_rate, spec.channels);
        ...
        info!("[VOICE] Extracted {} feature dimensions from {} samples", features.len(), samples.len());
        Ok(features)
    }
