...
impl AgentCore {
    pub fn new(api_url: &str) -> Self {
        Self {
            ...
            brain: Arc::new(AsyncMutex::new(Brain::new(api_url))),
    ...
    pub async fn process_youtube_intent(
        ...
        match source_tools::download_youtube(url, output_dir, login).await {
            Ok(info) => {
                self.log(&format!("[CORE] ✅ Video acquired: {}", info.title));
                ...
                if !intent.is_empty() {
                    ...
                    // Simple passthrough to smart editor for now
                    ...
                    match smart_editor::smart_edit(&info.local_path, intent, &out_path, Some(callback)).await {
                        Ok(res) => self.log(&format!("[CORE] ✅ {}", res)),
                        Err(e) => self.log(&format!("[CORE] ❌ Edit failed: {}", e)),
                    }
                } else {
                    if let Err(e) = std::fs::copy(&info.local_path, &out_path) {
            ...
            Err(e) => {
                self.log(&format!("[CORE] ❌ Download failed: {}", e));
                return Err(e);
            }
        }
    ...
    pub async fn voice_record(&self, output: Option<PathBuf>, duration: u32) -> Result<(), Box<dyn std::error::Error>> {
        ...
        match tokio::task::spawn_blocking(move || audio_io.record_to_file(&out_path, duration)).await? {
