...
impl ContentInjector {
    ...
    pub async fn inject_content(&self, input: &Path, output: &Path, moments: &[FunnyMoment]) -> Result<()> {
        if moments.is_empty() {
        ...
        let enable_expr: String = moments.iter()
            .take(50) 
            .map(|m| format!("between(t,{:.2},{:.2})", m.start_time, m.start_time + m.duration))
            .collect::<Vec<_>>()
            .join("+");
        ...
        let status = Command::new("ffmpeg")
            ...
            .arg(input)
            .arg("-vf").arg(&filter)
            .arg("-c:a").arg("copy") // Preserves audio (important!)
            .arg(output)
            .output()
        ...
        if !status.status.success() {
            ...
            // Print error
            anyhow::bail!("FFmpeg injection failed: {:?}", String::from_utf8_lossy(&status.stderr));
        }
